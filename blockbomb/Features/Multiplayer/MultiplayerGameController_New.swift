...
/// Multiplayer game controller extending GameController for turn-based gameplay
/// Handles multiplayer coordination, turn management, and Game Center integration
class MultiplayerGameController: GameController {
    ...
    // MARK: - Type Aliases
    typealias GameEndReason = MultiplayerGameController.GameEndReason
    ...
    private func setupMultiplayerObservers() {
        ...
        turnBasedMatchManager.currentMatchPublisher
        ...
        // Observe turn submission results
        turnBasedMatchManager.turnSubmissionResultPublisher
            .receive(on: DispatchQueue.main)
            .sink { [weak self] result in
                self?.turnManager.handleTurnSubmissionResult(success: result.success, error: result.error)
            }
            .store(in: &cancellables)
    ...
    func submitTurn() {
        ...
        // Encode and submit
        guard let encodedMatchData = MultiplayerGameState.encodeMatchData(updatedMatchState),
              let nextParticipant = turnManager.getNextParticipant(in: match) else {
